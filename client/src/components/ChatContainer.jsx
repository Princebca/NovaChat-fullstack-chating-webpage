...
const ChatContainer = () => {
  ...
  const { setSelectedUser, selectedUser, messages, sendMessages, getMessages } = useContext(ChatContext)
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-gray-200'>
        ...
        <p className='flex-1 text-lg text-white flex items-center gap-2'>
          ...
          {onlineUsers.includes(selectedUser._id) ? (<span className='w-2 h-2 rounded-full bg-green-500'></span>) : null}
        </p>
        <img onClick={() => setSelectedUser(null)} src={assets.arrow_icon} alt='' className='md:hidden max-w-7' />
        <img src={assets.help_icon} alt="" className='max-md:hidden max-w-5' />
      </div>
      {/* ----------------Chat Area----------------- */}
      <div className='flex flex-col h-[calc(100%-80px)] overflow-y-scroll p-3 pb-6'>
        {
      ...
      {/* ----------------bottom area----------------- */}
      <div className='absolute bottom-0 left-0 right-0 flex tems-center gap-2 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
          ...
          <input onChange={handleImageSend} type='file' id='image' accept='image/*' hidden />
