...
const Sidebar = () => {
  ...
  const filtedUsers = input ? users.filter((user) => user.fullName.toLowerCase(input.toLowerCase())) : users;
  ...
  return (
    <div className={`bg-[#8185b2]/10 h-full p-5 rounder-r-xl overflow-y-scroll text-white ${selectedUser ? 'mx-md:hidden' : ''} `}>
      <div className='pb-5'>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 px-4 py-3 mt-6'>
          <img src={assets.search_icon} className='w-3 cursor-pointer' alt='Search' />
          <input onChange={(e) => setInput(e.target.value)} type='text' placeholder='Search User...' className='bg-transparent border-none text-white text-xs outline-none placeholder-[#c8c8c8] flex-1' />
        </div>
      </div>
